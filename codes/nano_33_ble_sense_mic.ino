...
void setup()
{
    ...
    Serial.begin(115200);
    Serial1.begin(115200);            // initialize UART with baud rate of 115200
    pinMode(buzzer, OUTPUT);
    attachInterrupt(digitalPinToInterrupt(interruptPin), fire, CHANGE);
...
if (fire_detected){ //checks if fire is detected and sends results to the ESP-01 based on it
  Serial1.println('1');
  fire_detected=false;
}
  ei_printf("Starting inferencing in 2 seconds...\n");
    ...
    bool m = microphone_inference_record(); //here it gets the recording from the user  
    if (!m) {
...
static void fire(){
  digitalWrite(light, LOW);
  fire_detected=true; // Sending the value to ESP-01 }
}
